...
export function CompanyLifecyclePanel({
  ...
  async function change(action: "freeze" | "activate") {
    setBusy(true);
    setError("");
    try {
      if (action === "freeze") {
  ...
  return (
    <section className="company-card glass">
      <div className="panel-heading">
        ...
        <span className={`status-chip ${lifecycle.state === "active" ? "ok" : "warn"}`}>{stateLabel(lifecycle.state)}</span>
      </div>
...
function stateLabel(state: CompanyLifecycle["state"]) {
  if (state === "active") return "Активна";
  if (state === "frozen") return "Заморожена";
  return "Удаляется";
}
...
function stateDescription(lifecycle: CompanyLifecycle) {
  ...
  if (lifecycle.state === "frozen") {
    return lifecycle.purge_after
      ? `Чтение доступно, изменения и расход кредитов остановлены. Мягкое удаление ${formatDate(lifecycle.purge_after)}.`
      : "Чтение доступно, изменения и расход кредитов остановлены.";
  }
  return lifecycle.purge_after
